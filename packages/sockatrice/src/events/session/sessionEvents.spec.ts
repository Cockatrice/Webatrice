...
describe('listRooms', () => {
  ...
  it('does not call joinRoom when autojoinrooms is false', () => {
    WebClient.instance.clientOptions.autojoinrooms = false;
  ...
  it('calls joinRoom for autoJoin rooms when autojoinrooms is true', () => {
    WebClient.instance.clientOptions.autojoinrooms = true;
    listRooms(create(Event_ListRoomsSchema, {
      roomList: [
...
describe('serverIdentification', () => {
  ...
  it('REGISTER → calls register without hashedPassword when server does not support hashing', async () => {
    const opts = {
      host: 'h', port: '1', userName: 'alice', password: 'pw',
    ...
    (consumePendingOptions as Mock).mockReturnValue(opts);
    await serverIdentification(makeInfo());
  ...
  it('REGISTER with password salt → passes pre-computed hashedPassword', async () => {
    const opts = {
      host: 'h', port: '1', userName: 'alice', password: 'pw',
      email: 'a@b.com', country: 'US', realName: 'Al',
      reason: WebSocketConnectReason.REGISTER as const,
    ...
    expect(SessionCmds.register).toHaveBeenCalledWith(
      expect.objectContaining({ userName: 'alice', hashedPassword: 'hashedFromSalt' }),
      'pw',
    );
  });
  ...
  describe('LOGIN with password salt', () => {
    const setup = () => {
      (consumePendingOptions as Mock).mockReturnValue(makeLoginOptions());
      (passwordSaltSupported as Mock).mockReturnValue(1);
      (SessionCmds.requestPasswordSalt as Mock).mockClear();
      (SessionCmds.login as Mock).mockClear();
      (SessionCmds.disconnect as Mock).mockClear();
      (WebClient.instance.response.session.loginFailed as Mock).mockClear();
      serverIdentification(makeInfo({ serverOptions: 1 }));
    };

    it('salt callback with non-empty salt → login with pre-computed hashedPassword', async () => {
      setup();
      const [, onSalt] = (SessionCmds.requestPasswordSalt as Mock).mock.calls[0];
      await onSalt('xyz');
    ...
    it('salt callback with empty salt falls back to plain-password login', async () => {
      setup();
      const [, onSalt] = (SessionCmds.requestPasswordSalt as Mock).mock.calls[0];
      await onSalt('');
      expect(SessionCmds.login).toHaveBeenCalledWith(
    ...
    it('failure callback dispatches loginFailed and disconnects', () => {
      setup();
  ...
  describe('ACTIVATE_ACCOUNT with password salt', () => {
    ...
    const setup = () => {
      (consumePendingOptions as Mock).mockReturnValue(makeActivateOpts());
      (passwordSaltSupported as Mock).mockReturnValue(1);
      (SessionCmds.requestPasswordSalt as Mock).mockClear();
      (SessionCmds.activate as Mock).mockClear();
      (SessionCmds.disconnect as Mock).mockClear();
      (WebClient.instance.response.session.accountActivationFailed as Mock).mockClear();
      serverIdentification(makeInfo({ serverOptions: 1 }));
    };

    it('calls requestPasswordSalt and salt callback activates with hashedPassword', async () => {
      setup();
      expect(SessionCmds.requestPasswordSalt).toHaveBeenCalledWith(
      ...
      await onSalt('xyz');
      expect(SessionCmds.activate).toHaveBeenCalledWith(
        expect.objectContaining({ userName: 'alice', token: 'tok' }), 'pw', 'hashedFromSalt',
      );
    });

    it('salt callback with empty salt forwards undefined hashedPassword to activate', async () => {
      setup();
      const [, onSalt] = (SessionCmds.requestPasswordSalt as Mock).mock.calls[0];
      await onSalt('');
      expect(SessionCmds.activate).toHaveBeenCalledWith(
        expect.objectContaining({ userName: 'alice', token: 'tok' }), 'pw', undefined,
      );
    });
  ...
  describe('PASSWORD_RESET with password salt', () => {
    ...
    const setup = () => {
      (consumePendingOptions as Mock).mockReturnValue(makeResetOpts());
      (passwordSaltSupported as Mock).mockReturnValue(1);
      (SessionCmds.requestPasswordSalt as Mock).mockClear();
      (SessionCmds.forgotPasswordReset as Mock).mockClear();
      (SessionCmds.disconnect as Mock).mockClear();
      (WebClient.instance.response.session.resetPasswordFailed as Mock).mockClear();
      serverIdentification(makeInfo({ serverOptions: 1 }));
    };

    it('salt callback calls forgotPasswordReset with pre-computed hashedNewPassword', async () => {
      setup();
      const [, onSalt] = (SessionCmds.requestPasswordSalt as Mock).mock.calls[0];
      await onSalt('xyz');
      expect(SessionCmds.forgotPasswordReset).toHaveBeenCalledWith(
        expect.objectContaining({ userName: 'alice', token: 'tok', hashedNewPassword: 'hashedFromSalt' }),
        'newpw',
      );
    });

    it('salt callback with empty salt falls back to plain newPassword reset', async () => {
      setup();
      const [, onSalt] = (SessionCmds.requestPasswordSalt as Mock).mock.calls[0];
      await onSalt('');
      expect(SessionCmds.forgotPasswordReset).toHaveBeenCalledWith(
        expect.not.objectContaining({ hashedNewPassword: expect.anything() }),
        'newpw',
      );
    });

    it('failure callback dispatches resetPasswordFailed and disconnects', () => {
      setup();
      const [, , onFailure] = (SessionCmds.requestPasswordSalt as Mock).mock.calls[0];
      onFailure();
  ...
  it('unknown connection reason → updateStatus DISCONNECTED with reason number and disconnects', () => {
    (consumePendingOptions as Mock).mockReturnValue({
      host: 'h', port: '1', userName: 'alice',
      reason: 9999 as unknown as WebSocketConnectReason.LOGIN,
    });
    (SessionCmds.disconnect as Mock).mockClear();
    (SessionCmds.updateStatus as Mock).mockClear();
    serverIdentification(makeInfo());
    expect(SessionCmds.updateStatus).toHaveBeenCalledWith(
      StatusEnum.DISCONNECTED,
      expect.stringContaining('Unknown Connection Reason: 9999'),
    );
    expect(SessionCmds.disconnect).toHaveBeenCalled();
  });
});
