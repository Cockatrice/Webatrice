...
// password-hash utilities are mocked — everything in between is the
// production code path. hashPassword is mocked with a sentinel string so the
// test stays focused on flow wiring; byte-equivalence with the desktop client
// is covered by the hardcoded vectors in passwordHasher.spec.ts.
...
vi.mock('../../utils', async (importOriginal) => ({
  ...
  hashPassword: vi.fn().mockResolvedValue('xyzMOCKED_HASH'),
}));

import { create } from '@bufbuild/protobuf';
import { Mock } from 'vitest';
import { Event_ServerIdentificationSchema } from '../../generated';
...
beforeEach(() => {
  ...
  (hashPassword as Mock).mockResolvedValue('xyzMOCKED_HASH');
});

// Flush the microtask queue so the async requestPasswordSalt callback inside
// serverIdentification can resolve its `await hashPassword(...)` before the
// test inspects the resulting login command.
const flushMicrotasks = () => Promise.resolve();

describe('integration: fresh login on a hashed-password server', () => {
  it('loginSuccessful carries the hashedPassword produced by hashPassword(salt, password)', async () => {
    ...
    // 3. The async hash computation completes and login is dispatched.
    await flushMicrotasks();

    // 4. Login command was sent; resolve it with a minimal success response.
    invokeOnSuccess({ buddyList: [], ignoreList: [], userInfo: { name: 'alice' } });

    expect(hashPassword).toHaveBeenCalledWith('xyz', 'mypass');
    ...
    expect(WebClient.instance.response.session.loginSuccessful).toHaveBeenCalledWith({
      hashedPassword: 'xyzMOCKED_HASH',
    });
  });

  it('Command_Login carries hashedPassword and no plaintext password value', async () => {
    (consumePendingOptions as Mock).mockReturnValue(makeLoginOptions());
    (passwordSaltSupported as Mock).mockReturnValue(1);
...
describe('integration: server returns empty passwordSalt (effectively unsupported)', () => {
  it('falls back to a plain-password Command_Login', async () => {
    ...
    // Empty salt should bypass hashing entirely.
    expect(hashPassword).not.toHaveBeenCalled();
  });

  it('does not surface as a login failure', async () => {
    (consumePendingOptions as Mock).mockReturnValue(makeLoginOptions());
    (passwordSaltSupported as Mock).mockReturnValue(1);
