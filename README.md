...
The Cockatrice web client â€” a React/TypeScript SPA that connects to a Servatrice server over a WebSocket.

## Application Architecture

![Application Architecture](architecture/simple.png?raw=true "Application Architecture")

For the full set of diagrams (detailed layer map + command/response/event sequence) and the `npm run diagram` scripts that regenerate them, see [architecture/](architecture/). For prose â€” WebSocket layering, Redux store shape, test conventions â€” see [.github/instructions/root.instructions.md](.github/instructions/root.instructions.md).
...
- `npm start` â€” start the Vite dev server (runs `prebuild.js` first via `prestart`)
- `npm run build` â€” production build into `build/` (also runs the prebuild hooks)
- `npm run preview` â€” serve the built `build/` output locally to smoke-test a production build

### Tests

- `npm test` â€” one-shot Vitest run (unit specs)
- `npm run test:watch` â€” Vitest in watch mode
- `npm run test:integration` â€” integration specs via `vitest.integration.config.ts`
- `npm run test:coverage` / `npm run test:integration:coverage` â€” the above with v8 coverage
...
- `npm run lint` / `npm run lint:fix` â€” ESLint over `src/`
- `npm run golden` â€” `lint` + `test` + `test:integration`; the fast CI-equivalent gate to run before declaring work done

### i18n

- `npm run translate` â€” re-run the i18n merge only (`prebuild.js -i18nOnly`)
...
- [.github/instructions/root.instructions.md](.github/instructions/root.instructions.md) â€” architecture deep dive, conventions, and domain-knowledge invariants for working in this directory (the canonical AI-tool instruction surface for this package)
- [Vite docs](https://vite.dev/guide/) Â· [React docs](https://react.dev/) Â· [Vitest docs](https://vitest.dev/)
