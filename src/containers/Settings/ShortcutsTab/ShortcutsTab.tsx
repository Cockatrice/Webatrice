  ...
  const filteredGroups = useMemo(() => {
    ...
    for (const groupId of GROUP_ORDER) {
      out[groupId] = grouped[groupId].filter((id) => {
        const label = t(`Settings.shortcuts.action.${id}`).toLowerCase();
  ...
  return (
    <Paper>
      <div className="ShortcutsTab__header">
        <Typography variant="h6">{t('Settings.shortcuts.title')}</Typography>
      ...
      {!hasResults ? (
        <div className="ShortcutsTab__noResults">{t('Settings.shortcuts.noResults')}</div>
      ) : (
          ...
          return (
            <Accordion key={groupId} defaultExpanded>
              <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Typography>{t(`Settings.shortcuts.group.${groupId}`)}</Typography>
              </AccordionSummary>
              <AccordionDetails>
                {ids.map((id) => (
                  <ShortcutsRow
