...
import { makeGame, makeUser } from '../../testing/fixtures/rooms';
...
describe('gameMatchesFilters', () => {
  ...
  it('hideOpenDecklistGames hides games with shareDecklistsOnLoad set', () => {
    const game = makeGame({ maxPlayers: 4, shareDecklistsOnLoad: true });
    const filters = { ...DEFAULT_GAME_FILTERS, hideOpenDecklistGames: true };
    expect(gameMatchesFilters(game, filters, ctx())).toBe(false);
  });

  it('hideNotBuddyCreatedGames hides games whose creator is not a buddy', () => {
    const game = makeGame({ maxPlayers: 4, creatorInfo: makeUser({ name: 'stranger' }) });
    const filters = { ...DEFAULT_GAME_FILTERS, hideNotBuddyCreatedGames: true };
    expect(gameMatchesFilters(game, filters, ctx({ isUserBuddy: () => false }))).toBe(false);
    expect(gameMatchesFilters(game, filters, ctx({ isUserBuddy: (n) => n === 'stranger' }))).toBe(true);
  });

  it('showOnlyIfSpectatorsCanWatch hides spectator-password games unless showSpectatorPasswordProtected is set', () => {
    const game = makeGame({ maxPlayers: 4, spectatorsAllowed: true, spectatorsNeedPassword: true });
    const baseFilters = { ...DEFAULT_GAME_FILTERS, showOnlyIfSpectatorsCanWatch: true };
    expect(gameMatchesFilters(game, baseFilters, ctx())).toBe(false);
    expect(gameMatchesFilters(game, { ...baseFilters, showSpectatorPasswordProtected: true }, ctx())).toBe(true);
  });

  it('showOnlyIfSpectatorsCanSeeHands further requires spectatorsOmniscient', () => {
    const filters = {
      ...DEFAULT_GAME_FILTERS,
      showOnlyIfSpectatorsCanWatch: true,
      showOnlyIfSpectatorsCanSeeHands: true,
    };
    expect(gameMatchesFilters(
      makeGame({ maxPlayers: 4, spectatorsAllowed: true, spectatorsOmniscient: false }),
      filters,
      ctx(),
    )).toBe(false);
    expect(gameMatchesFilters(
      makeGame({ maxPlayers: 4, spectatorsAllowed: true, spectatorsOmniscient: true }),
      filters,
      ctx(),
    )).toBe(true);
  });
