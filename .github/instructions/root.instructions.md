...
Protobuf bindings are produced by Sockatrice and consumed via `@app/generated`. [src/types/](../../src/types/) re-exports them namespaced as `Data` (raw proto), `Enriched` (UI/domain composition — proto extended with client-only sibling fields), and `App` (pure client types). Consumer pattern: `import { Data, Enriched, App } from '@app/types'`. **UI, store, hooks, and api code must import proto types through `@app/types`, never `@app/generated` directly.**

Websocket protocol/transport types (`StatusEnum`, `WebSocketConnectReason`, the `*ConnectOptions` family, signal-payload contexts, `GameEventMeta`, `I*Request`/`I*Response` contracts, `WebClientConfig`) live at `@app/websocket/types` as the `WebsocketTypes` namespace. This is the only public surface for those types — never re-export them through `Enriched`.
...
1. UI layers call `useWebClient()` ([src/hooks/useWebClient.tsx](../../src/hooks/useWebClient.tsx)) to get the Sockatrice `WebClient`, then `client.request.<scope>.<method>(…)`. Never import from `@app/websocket` in UI code directly (`@app/websocket/types` is fine — type-only). `new WebClient(...)` is called only inside `WebClientProvider`, never at module load.
2. Sockatrice fires response callbacks into the `IWebClientResponse` instance built by [src/api/response/](../../src/api/response/) `createWebClientResponse()`. The per-scope `*ResponseImpl` classes (session / room / game / admin / moderator) are the only place outside `src/store/*.dispatch.ts` that may dispatch to the Redux store.
3. Only `*.dispatch.ts` helpers inside `src/store/` and `*ResponseImpl` classes may touch the Redux store.
...
One `*ResponseImpl` class per inbound scope: session, room, game, admin, moderator. `createWebClientResponse()` ([src/api/response/index.ts](../../src/api/response/index.ts)) bundles them into the `IWebClientResponse` Sockatrice consumes. [src/api/clientConfig.ts](../../src/api/clientConfig.ts) supplies `CLIENT_CONFIG` (clientid, clientver, clientfeatures) and `CLIENT_OPTIONS` (autojoin, keepalive). **UI code never imports from `src/api/` directly — use `useWebClient()`.**

### State (`src/store/`)

Slices: `server/`, `rooms/`, `game/`, `actions/`. Consumers import through the `@app/store` barrel (`GameSelectors`, `GameDispatch`, `GameTypes`, same for `Server`/`Rooms`). **Don't deep-import from `src/store/<slice>/*` — add the symbol to the barrel's `index.ts` instead.** Slice shapes, reducer-author hazards, and the `Enriched` sibling-field convention live in [store.instructions.md](store.instructions.md).
...
**Schema migrations can't change a primary key in place.** Dexie throws "Not yet support for changing primary key" — drop the affected tables and recreate under the new key, accepting a clean re-import. The v1→v2→v3 migration of `cards`/`sets` to the XSD v4 shape is the worked example.
