...
**Immer can't draft protobuf-es messages — to change a stored message, reassign a fresh clone.** proto2 messages are built with `Object.create(prototype)` (to track field presence), so their non-`Object.prototype` prototype makes Immer's `isDraftable` skip them. A `card.field = X` mutation in a reducer passes straight through untracked: the stored reference never changes, so the WeakMap selectors in [game.selectors.ts](../../packages/datatrice/src/store/games/game.selectors.ts) (`zoneCardsCache`, `attachmentsByParentCache`) keep serving the cached value and the UI doesn't re-render. **Fix:** never mutate a stored message in place. Reassign a fresh copy via `cloneWith(schema, message, patch)` in [src/common/cloneWith.ts](../../packages/datatrice/src/common/cloneWith.ts) (e.g. `zone.byId[cardId] = cloneWith(ServerInfo_CardSchema, card, fields)`); for `mergeSetFields`-style merges, `clone` first then merge then reassign. This applies to **all three slices** (`games`, `rooms`, `server`).
