...
- The app integration suite ([integration/src/app/helpers.tsx](../../integration/src/app/helpers.tsx)) provides `WebClientContext` directly rather than wrapping in `<WebClientProvider>` (the shared setup already instantiates the singleton in `beforeEach`) and passes the **real** `@app/store` so assertions hit the same store the production `WebClient` dispatches against.
