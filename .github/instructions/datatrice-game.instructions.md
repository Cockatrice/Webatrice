...
**`Event_MoveCard` for an opponent's hidden card carries `card_id = -1` and an unresolvable position.** When a card moves between an opponent's hidden zones (e.g. hand → library during a mulligan), the server can't reveal its identity, so `card_id` is `-1` and the position doesn't resolve against the empty `order`. The `cardMoved` listener can't move an identified card, so for a genuine cross-zone move it adjusts `cardCount` by `-1`/`+1` on the two zones instead. Without this, opponent hand/library totals desync (a same-size mulligan would leave the hand at `2×` and the library short by a full hand).
