...
- **Arrow from local-hand → non-hand plays the card AND draws the arrow.** Mirrors desktop `arrow_item.cpp:223-282` (`ArrowDragItem::mouseReleaseEvent`): client sends `moveCard` (via `playCardViaTableRow`) then `createArrow` with `startZone` rewritten to the post-play zone (TABLE, or STACK for `tablerow === 3`) and the original hand-side `startCardId` — Servatrice resolves it against the freshly-moved card. Lives in both right-click-drag (`useGameArrowInteractions`) and click-target (`handleCardClick`); both must stay in sync.
